...
router.get('/games/:id', requireToken, (req, res, next) => {
  ...
  // if `findById` is succesful, respond with 200 and "example" JSON
    .then((games) => res.status(200).json({ games: games.toObject() }))
  // if an error occurs, pass it to the handler
    .catch(next)
})
...
router.post('/games', requireToken, (req, res, next) => {
  ...
  // respond to succesful `create` with status 201 and JSON of new "example"
    .then((games) => {
      res.status(201).json({ games: games.toObject() })
    })
...
router.patch('/games/:id', requireToken, removeBlanks, (req, res, next) => {
  ...
  Games.findById(req.params.id)
    ...
    .then((games) => {
      ...
      requireOwnership(req, games)

      // pass the result of Mongoose's `.update` to the next `.then`
      return games.updateOne(req.body.games)
    })
